...
console.log("params",   process.argv)
 if(process.argv[0]!= undefined) copyroutes();
function copyroutes(){
process.chdir(process.argv[2])
fs.readdir(process.argv[2],{}, function(err,data){
data.forEach(function(x,y){
    //console.log('process.argv[2]+x',process.argv[2]+x)
    fs.readdir(process.argv[2]+x+"/",{}, function(errx,datax){
        if(datax){
             datax.forEach(function(u,v){
                 if(u.indexOf("routes")<0){
                    fs.readFile('./routes.js', 'utf8', function (err, file) {
                        console.log(__dirname,"DDD")
                        if (err) throw err;
              
                         fs.writeFileSync(process.argv[2]+x, file, 'utf8', function (err) {
                            if (err) throw err;
                            console.log('Saved!');
                        });
                    });                     
                    /* fs.copyFile("./routes.js",process.argv[2]+x, function(err,d){
                         console.log('err',err)
                         console.log("data",d)
                     })*/
                    console.log('datax no contiene',datax )
                }
            })
            /**/
        }
        //console.log(errx,'datax',datax)
    })
})
})
//copyFile("./routes.js","./")
